...
class Crawler(object):
    ...
    def parse(self, id_block):
        try:
            ...
            data_ids_pattern = re.compile('<a data-id="([\s\S]+?)"')
            data_ids_list = data_ids_pattern.findall(page["data"]["summary"])
            if data_ids_list is not None:
                for data_id in data_ids_list:
                    print("o" * 100)
                    self.save_to_mongo(self.id_mongo, {"id": data_id, "tag": 0})
            page["data"]["tag"] = 0
            self.save_to_mongo(self.data_mongo, page["data"])
            self.id_mongo.update_one({"_id": id_block["_id"]}, {"$set": {"tag": 1}})
        except Exception as e:
            print(e)
            try:
                if "词条不存在" in str(page):
                    self.id_mongo.update_one({"_id": id_block["_id"]}, {"$set": {"tag": 10}})
            except Exception as e:
                pass
            sem.release()
            try:
